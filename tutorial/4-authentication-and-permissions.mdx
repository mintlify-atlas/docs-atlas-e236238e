...
description: "Learn how to add authentication and permissions to control access to your API"
---
...
```python snippets/permissions.py
from rest_framework import permissions
...
<Steps>
  <Step title="Inherit from BasePermission">
    All custom permissions should inherit from `rest_framework.permissions.BasePermission`.
  </Step>
  
  <Step title="Implement permission methods">
    You can implement `has_permission(self, request, view)` for view-level permissions and/or `has_object_permission(self, request, view, obj)` for object-level permissions.
  </Step>
  
  <Step title="Return True or False">
    Return `True` if the request should be granted access, and `False` otherwise.
  </Step>
  
  <Step title="Use SAFE_METHODS">
    `permissions.SAFE_METHODS` is a tuple containing `'GET'`, `'HEAD'`, and `'OPTIONS'` - the read-only methods.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="SessionAuthentication">
...
In [part 5](/tutorial/5-relationships-and-hyperlinked-apis) of the tutorial we'll look at how we can tie everything together by creating an HTML endpoint for our highlighted snippets, and improve the cohesion of our API by using hyperlinking for the relationships within the system.
